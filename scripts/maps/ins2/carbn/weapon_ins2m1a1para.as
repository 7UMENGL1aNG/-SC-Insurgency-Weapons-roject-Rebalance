...
string V_MODEL = "models/ins2fair/wpn/m1a1p/v_m1a1p.mdl";
...
uint DAMAGE     	= 1.2 * 30;
uint DAMAGE_MELEE 	= 2 * 12 * 15;
uint SLOT       	= 3;
uint POSITION   	= 9;
float RPM_AIR   	= 0.12f / 1.2f; //Rounds per minute in air
float RPM_WTR   	= 0.22f / 1.2f; //Rounds per minute in water
...
class weapon_ins2m1a1para : ScriptBasePlayerWeaponEntity, INS2BASE::WeaponBase, INS2BASE::MeleeWeaponBase
{
	...
	bool Deploy()
	{
		...
		{
			m_WasDrawn = true;
			return Deploy( V_MODEL, P_MODEL, DRAW_FIRST, "m16", GetBodygroup(), (86.0/51.0) );
		}
		else
			return Deploy( V_MODEL, P_MODEL, DRAW, "m16", GetBodygroup(), (22.0/48.0) );
	}
	...
	void Reload()
	{
		...
		{
			(self.m_iClip == 0) ? Reload( MAX_CLIP, RELOAD_EMPTY, (170.0/56.0), GetBodygroup() ) : Reload( MAX_CLIP, RELOAD, (115.0/56.0), GetBodygroup() );
			canReload = false;
		}
