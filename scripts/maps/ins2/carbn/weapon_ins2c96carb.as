...
string V_MODEL = "models/ins2fair/wpn/c96c/v_c96c.mdl";
...
uint SLOT       	= 3;
uint POSITION   	= 4;
float RPM_AIR   	= 1.2 * 1000; //Rounds per minute in air
float RPM_WTR   	= 1.2 * 700; //Rounds per minute in water
...
class weapon_ins2c96carb : ScriptBasePlayerWeaponEntity, INS2BASE::WeaponBase
{
	...
	bool Deploy()
	{
		...
		{
			m_WasDrawn = true;
			return Deploy( V_MODEL, P_MODEL, DRAW_FIRST, "mp5", GetBodygroup(), (54.0/53.0) );
		}
		else
			return Deploy( V_MODEL, P_MODEL, (self.m_iClip > 0) ? DRAW : DRAW_EMPTY, "mp5", GetBodygroup(), (25.0/56.0) );
	}
	...
	{
		if( self.m_iClip > 0 )
			ChangeFireMode( (WeaponADSMode == INS2BASE::IRON_IN) ? IRON_FIRESELECT : FIRESELECT, GetBodygroup(), INS2BASE::SELECTFIRE_AUTO, INS2BASE::SELECTFIRE_SEMI, (30.0/62) );
		else
			ChangeFireMode( (WeaponADSMode == INS2BASE::IRON_IN) ? IRON_FIRESELECT_EMPTY : FIRESELECT_EMPTY, GetBodygroup(), INS2BASE::SELECTFIRE_AUTO, INS2BASE::SELECTFIRE_SEMI, (30.0/62) );
		...
		{
			(self.m_iClip == 0) ? Reload( MAX_CLIP, RELOAD_EMPTY, (142.0/52.0), GetBodygroup() ) : Reload( MAX_CLIP, RELOAD, (100.0/49.0), GetBodygroup() );
			canReload = false;
		}
