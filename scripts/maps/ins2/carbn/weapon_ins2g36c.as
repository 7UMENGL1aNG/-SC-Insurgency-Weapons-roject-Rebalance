...
string V_MODEL = "models/ins2fair/wpn/g36c/v_g36c.mdl";
...
uint SLOT       	= 3;
uint POSITION   	= 8;
float RPM_AIR   	= 1.2 * 741; //Rounds per minute in air
...
class weapon_ins2g36c : ScriptBasePlayerWeaponEntity, INS2BASE::WeaponBase
{
	...
	bool Deploy()
	{
		...
		{
			m_WasDrawn = true;
			return Deploy( V_MODEL, P_MODEL, DRAW_FIRST, "m16", GetBodygroup(), (85.0/58.0) );
		}
		else
			return Deploy( V_MODEL, P_MODEL, (self.m_iClip > 0) ? DRAW : DRAW_EMPTY, "m16", GetBodygroup(), (19.0/48.0) );
	}
	...
	{
		if( self.m_iClip > 0 )
			ChangeFireMode( (WeaponADSMode == INS2BASE::IRON_IN) ? IRON_FIRESELECT : FIRESELECT, GetBodygroup(), INS2BASE::SELECTFIRE_AUTO, INS2BASE::SELECTFIRE_SEMI, (30.0/70.0) );
		else
			ChangeFireMode( (WeaponADSMode == INS2BASE::IRON_IN) ? IRON_FIRESELECT_EMPTY : FIRESELECT_EMPTY, GetBodygroup(), INS2BASE::SELECTFIRE_AUTO, INS2BASE::SELECTFIRE_SEMI, (30.0/70.0) );
		...
		if( m_reloadTimer < g_Engine.time )
		{
			if( self.m_iClip <= 0 )
			{
				Reload( MAX_CLIP, RELOAD_EMPTY, (153.0/50.0), GetBodygroup() );
			}
			else
			{
				Reload( MAX_CLIP, RELOAD, (140.0/50.0), GetBodygroup() );
			}
