...
string V_MODEL = "models/ins2fair/wpn/fg42/v_fg42.mdl";
...
float RPM_WTR   	= 1.2 * 675; //Rounds per minute in water
...
class weapon_ins2fg42 : ScriptBasePlayerWeaponEntity, INS2BASE::WeaponBase, INS2BASE::BipodWeaponBase
{
	...
	bool Deploy()
	{
		...
		{
			m_WasDrawn = true;
			return Deploy( V_MODEL, P_MODEL, DRAW_FIRST, "m16", GetBodygroup(), (74.0/56.0) );
		}
		else
			return Deploy( V_MODEL, P_MODEL, (self.m_iClip > 0) ? DRAW : DRAW_EMPTY, "m16", GetBodygroup(), (22.0/56.0) );
	}
	...
	void TertiaryAttack()
	{
		...
		{
			if( self.m_iClip > 0 )
				DeployBipod( BIPOD_IRON_IN, BIPOD_IRON_OUT, GetBodygroup(), (64.0/50.0), (73.0/50.0) );
			else
				DeployBipod( BIPOD_IRON_IN_EMPTY, BIPOD_IRON_OUT_EMPTY, GetBodygroup(), (64.0/50.0), (73.0/50.0) );
		}
		else
		{
			if( self.m_iClip > 0 )
				DeployBipod( BIPOD_IN, BIPOD_OUT, GetBodygroup(), (64.0/50.0), (73.0/50.0) );
			else
				DeployBipod( BIPOD_IN_EMPTY, BIPOD_OUT_EMPTY, GetBodygroup(), (64.0/50.0), (73.0/50.0) );
		}
	}
	...
	void Reload()
	{
		...
		{
			if( self.m_iClip > 0 )
				ChangeFireMode( (WeaponADSMode == INS2BASE::IRON_IN) ? IRON_FIRESELECT : FIRESELECT, GetBodygroup(), INS2BASE::SELECTFIRE_AUTO, INS2BASE::SELECTFIRE_SEMI, (29.0/60.0) );
			else
				ChangeFireMode( (WeaponADSMode == INS2BASE::IRON_IN) ? IRON_FIRESELECT_EMPTY : FIRESELECT_EMPTY, GetBodygroup(), INS2BASE::SELECTFIRE_AUTO, INS2BASE::SELECTFIRE_SEMI, (29.0/60.0) );
		}
		else if( WeaponBipodMode == INS2BASE::BIPOD_DEPLOYED )
		{
			if( self.m_iClip > 0 )
				ChangeFireMode( (WeaponADSMode == INS2BASE::IRON_IN) ? BIPOD_IRON_FIRESELECT : BIPOD_FIRESELECT, GetBodygroup(), INS2BASE::SELECTFIRE_AUTO, INS2BASE::SELECTFIRE_SEMI, (29.0/60.0) );
			else
				ChangeFireMode( (WeaponADSMode == INS2BASE::IRON_IN) ? BIPOD_IRON_FIRESELECT_EMPTY : BIPOD_FIRESELECT_EMPTY, GetBodygroup(), INS2BASE::SELECTFIRE_AUTO, INS2BASE::SELECTFIRE_SEMI, (29.0/60.0) );
		}
		...
		{
			if( WeaponBipodMode == INS2BASE::BIPOD_UNDEPLOYED )
				(self.m_iClip == 0) ? Reload( MAX_CLIP, RELOAD_EMPTY, (214.0/52.0), GetBodygroup() ) : Reload( MAX_CLIP, RELOAD, (166.0/52.0), GetBodygroup() );
			else if( WeaponBipodMode == INS2BASE::BIPOD_DEPLOYED )
				(self.m_iClip == 0) ? Reload( MAX_CLIP, BIPOD_RELOAD_EMPTY, (223.0/52.0), GetBodygroup() ) : Reload( MAX_CLIP, BIPOD_RELOAD, (166.0/52.0), GetBodygroup() );
			canReload = false;
		}
