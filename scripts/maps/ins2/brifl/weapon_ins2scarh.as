// Insurgency's FN SCAR-H
/* Model Credits
/ Model: End of Days, ImBrokeRu (Magazine), Norman The Loli Pirate (Edits)
/ Textures: Racer445, Bull5hit (Magazine), Norman The Loli Pirate (Edits)
/ Animations: Mr. Brightside, D.N.I.O. 071 (Heavy Edits)
/ Sounds: KernCore (Mix of Navaro, Tripwire Interactive, D.I.C.E.), D.N.I.O. 071 (Conversion to .ogg format)
...
namespace INS2_SCARH
{

// Animations
enum INS2_SCARH_Animations
{
	IDLE = 0,
	IDLE_EMPTY,
	DRAW_FIRST,
...
// Models
string W_MODEL = "models/ins2/wpn/scar/w_scar.mdl";
string V_MODEL = "models/ins2fair/wpn/scar/v_scar.mdl";
string P_MODEL = "models/ins2/wpn/scar/p_scar.mdl";
string A_MODEL = "models/ins2/ammo/mags2.mdl";
int MAG_BDYGRP = 6;
// Sprites
string SPR_CAT = "ins2/brf/"; //Weapon category used to get the sprite's location
// Sounds
string SHOOT_S = "ins2/wpn/scar/shoot.ogg";
string EMPTY_S = "ins2/wpn/scar/empty.ogg";
// Information
int MAX_CARRY   	= 1000;
int MAX_CLIP    	= 20;
...
class weapon_ins2scarh : ScriptBasePlayerWeaponEntity, INS2BASE::WeaponBase
{
	...
	private array<string> Sounds = {
		"ins2/wpn/scar/bltbk.ogg",
		"ins2/wpn/scar/bltrel.ogg",
		"ins2/wpn/scar/rof.ogg",
		"ins2/wpn/scar/magin.ogg",
		"ins2/wpn/scar/magout.ogg",
		"ins2/wpn/scar/magrel.ogg",
		EMPTY_S,
		SHOOT_S
	};
	...
	bool Deploy()
	{
		...
		{
			m_WasDrawn = true;
			return Deploy( V_MODEL, P_MODEL, DRAW_FIRST, "m16", GetBodygroup(), (72.0/56.0) );
		}
		else
			return Deploy( V_MODEL, P_MODEL, (self.m_iClip == 0) ? DRAW_EMPTY : DRAW, "m16", GetBodygroup(), (22.0/48.0) );
	}
	...
	void PrimaryAttack()
	{
		...
		ShootWeapon( SHOOT_S, 1, VecModAcc( VECTOR_CONE_1DEGREES, 2.25f, 0.65f, 1.95f ), (m_pPlayer.pev.waterlevel == WATERLEVEL_HEAD) ? 1024 : 10240, DAMAGE, true );
		...
		if( WeaponADSMode != INS2BASE::IRON_IN )
		{
			if( m_pPlayer.pev.velocity.Length2D() > 0 )
				KickBack( 1.55, 0.25, 0.225, 0.05, 6.0, 1.5, 7 );
			else if( !( m_pPlayer.pev.flags & FL_ONGROUND != 0 ) )
				KickBack( 2.0, 0.8, 0.5, 0.35, 9.0, 6.0, 5 );
			else if( m_pPlayer.pev.flags & FL_DUCKING != 0 )
				KickBack( 0.95, 0.2, 0.15, 0.025, 4.0, 0.95, 9 );
			else
				KickBack( 1.05, 0.235, 0.175, 0.0375, 5.0, 1.25, 8 );
		}
		else
		{
			PunchAngle( Vector( Math.RandomFloat( -2.6, -2.4 ), (Math.RandomLong( 0, 1 ) < 0.5) ? -0.3f : 0.3f, Math.RandomFloat( -0.5, 0.5 ) ) );
		}
		...
		ShellEject( m_pPlayer, m_iShell, (WeaponADSMode != INS2BASE::IRON_IN) ? Vector( 19, 6.5, -8 ) : Vector( 19, 2, -4 ) );
	}
	...
	void Reload()
	{
		if( self.m_iClip > 0 )
			...
			ChangeFireMode( (WeaponADSMode == INS2BASE::IRON_IN) ? IRON_FIRESELECT_EMPTY : FIRESELECT_EMPTY, GetBodygroup(), INS2BASE::SELECTFIRE_AUTO, INS2BASE::SELECTFIRE_SEMI, (25.0/60.0) );
		...
		{
			(self.m_iClip == 0) ? Reload( MAX_CLIP, RELOAD_EMPTY, (150.0/42.0), GetBodygroup() ) : Reload( MAX_CLIP, RELOAD, (105.0/42.0), GetBodygroup() );
			canReload = false;
		}
...
class SCARH_MAG : ScriptBasePlayerAmmoEntity, INS2BASE::AmmoBase
{
	void Spawn()
	{
		Precache();
...
string GetAmmoName()
{
	return "ammo_ins2scarh";
}

string GetName()
{
	return "weapon_ins2scarh";
}

void Register()
{
	g_CustomEntityFuncs.RegisterCustomEntity( "INS2_SCARH::weapon_ins2scarh", GetName() ); // Register the weapon entity
	g_CustomEntityFuncs.RegisterCustomEntity( "INS2_SCARH::SCARH_MAG", GetAmmoName() ); // Register the ammo entity
