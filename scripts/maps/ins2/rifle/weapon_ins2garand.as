...
string V_MODEL = "models/ins2fair/wpn/garand/v_garand.mdl";
...
uint SLOT       	= 6;
uint POSITION   	= 5;
float RPM_AIR   	= 0.2f / 1.2f; //Rounds per minute in air
float RPM_WTR   	= 0.35f / 1.2f; //Rounds per minute in water
...
class weapon_ins2garand : ScriptBasePlayerWeaponEntity, INS2BASE::WeaponBase, INS2BASE::MeleeWeaponBase
{
	...
	bool Deploy()
	{
		...
		{
			m_WasDrawn = true;
			return Deploy( V_MODEL, P_MODEL, DRAW_FIRST, "sniper", GetBodygroup(), (60.0/56.0) );
		}
		else
			return Deploy( V_MODEL, P_MODEL, (self.m_iClip == 0) ? DRAW_EMPTY : DRAW, "sniper", GetBodygroup(), (35.0/56.0) );
	}
	...
	void Reload()
	{
		...
		{
			if( self.m_iClip > 0 )
				Reload( MAX_CLIP, RELOAD, (164.0/49.0), SetBodygroup() );
			else
			{
				/*switch( ( m_iSwing++ ) % 2 )
				{
				...
				{
					Reload( MAX_CLIP, RELOAD_NULL, (191.0/49.0), SetBodygroup() );
					SetThink( ThinkFunction( this.Secret ) );
					self.pev.nextthink = g_Engine.time + 2.28;
				}
				else
					Reload( MAX_CLIP, RELOAD_EMPTY, (116.0/49.0), SetBodygroup() );
			}
			canReload = false;
		}
