...
string V_MODEL = "models/ins2fair/wpn/kar98/v_k98.mdl";
...
uint DAMAGE     	= 1.2 * 65;
uint DAMAGE_GL  	= 1.2 * 155;
uint SLOT       	= 6;
uint POSITION   	= 4;
float RPM_AIR   	= 1.3f / 1.2f; //Rounds per minute in air
...
class weapon_ins2kar98k : ScriptBasePlayerWeaponEntity, INS2BASE::WeaponBase
{
	...
	bool Deploy()
	{
		...
		{
			m_WasDrawn = true;
			return Deploy( V_MODEL, P_MODEL, DRAW_FIRST, "sniper", GetBodygroup(), (121.0/56.0) );
		}
		else
		{
			if( WeaponGLMode == INS2BASE::GL_NOTAIMED )
			{
				if( m_bCanBolt )
				{
					...
					self.pev.nextthink = g_Engine.time + (21.0/50.0);
				}
				return Deploy( V_MODEL, P_MODEL, (self.m_iClip <= 0 || m_bCanBolt) ? DRAW_EMPTY : DRAW, "sniper", GetBodygroup(), (22.0/50.0) );
			}
			else
				return Deploy( V_MODEL, P_MODEL, (self.m_iClip > 0) ? GL_DRAW : GL_DRAW_EMPTY, "sniper", GetBodygroup(), (29.0/51.0) );
		}
	}
	...
	void BoltBackThink()
	{
		...
		if( self.m_iClip > 0 ) //Checking again in case the player depleted the ammo and didn't let go of the attack button
		{
			...
			self.m_flNextPrimaryAttack = self.m_flNextSecondaryAttack = self.m_flNextTertiaryAttack = WeaponTimeBase() + (36.0/42.0);
			self.m_flTimeWeaponIdle = WeaponTimeBase() + (43.0/42.0);

			SetThink( ThinkFunction( this.ShellEjectThink ) );
			self.pev.nextthink = WeaponTimeBase() + 0.42;
		}
		m_bCanBolt = false;
	}
	...
	void PrimaryAttack()
	{
		if( WeaponGLMode == INS2BASE::GL_NOTAIMED )
		{
			...
			if( self.m_iClip > 0 )
			{
				m_bCanBolt = true;
				SetThink( ThinkFunction( this.ShellEjectThink ) );
				self.pev.nextthink = WeaponTimeBase() + (29.0/42.0);
			}
			else
				SetThink( null );
		}
	...
	void ItemPostFrame()
	{
		...
		if( m_fRifleReload )
		{
			if( (CheckButton() || (self.m_iClip >= MAX_CLIP && m_pPlayer.pev.button & IN_RELOAD != 0)) && m_flNextReload <= g_Engine.time )
			{
				...
				self.m_flTimeWeaponIdle = g_Engine.time + (48.0/50.0);
				self.m_flNextPrimaryAttack = self.m_flNextSecondaryAttack = self.m_flNextTertiaryAttack = g_Engine.time + (48.0/50.0);
				m_fRifleReload = false;
			}
		}
		...
		if( WeaponGLMode == INS2BASE::GL_NOTAIMED )
		{
			...
			if( m_reloadTimer < g_Engine.time )
			{
				...
				if( m_bIsSwitching )
				{
					if( self.m_iClip2 == 0 )
					{
						GrenadeLauncherSwitch( (self.m_iClip > 0) ? GL_IN : GL_IN_EMPTY, (self.m_iClip == 0) ? GL_OUT_EMPTY_BOTH : GL_OUT_EMPTY_CLIP2, GetBodygroup(), (235.0/62.0), (79.0/56.0) );
					}
					else
					{
						GrenadeLauncherSwitch( (self.m_iClip > 0) ? GL_IN : GL_IN_EMPTY, (self.m_iClip == 0) ? GL_OUT_EMPTY_CLIP1 : GL_OUT, GetBodygroup(), (235.0/62.0), (178.0/62.0) );
					}

					SetThink( ThinkFunction( this.GiveAmmo ) );
					self.pev.nextthink = g_Engine.time + (181.0/62.0);
					m_bIsSwitching = false;
				...
				{
					Reload( MAX_CLIP, (self.m_iClip > 0) ? RELOAD_CLIP : RELOAD_CLIP_EMPTY, (150.0/55.0), SetBodygroup() );
				...
				else
				{
					...
					if( !m_fRifleReload )
					{
						...
						m_pPlayer.m_flNextAttack = (43.0/50.0); //Always uses a relative time due to prediction
						self.m_flTimeWeaponIdle = self.m_flNextSecondaryAttack = self.m_flNextPrimaryAttack = self.m_flNextTertiaryAttack = WeaponTimeBase() + (43.0/50.0);
					...
					else if( m_fRifleReload )
					{
						...
						m_pPlayer.m_flNextAttack = (37.0/53.0);
						m_flNextReload = WeaponTimeBase() + (37.0/53.0);
						self.m_flTimeWeaponIdle = self.m_flNextPrimaryAttack = self.m_flNextSecondaryAttack = self.m_flNextTertiaryAttack = WeaponTimeBase() + (37.0/53.0);
		...
		else
		{
			...
			if( m_reloadTimer < g_Engine.time )
			{
				...
				if( m_bIsSwitching )
				{
					...
					SetThink( ThinkFunction( this.GiveAmmo ) );
					self.pev.nextthink = g_Engine.time + (121.0/62.0);
					m_bIsSwitching = false;
					//Doing Reload Stuff here just so you don't reload the weapon without your own input
					m_reloadTimer = 0;
					canReload = false;
				...
				ReloadSecondary( MAX_CLIP2, GL_RELOAD, (182.0/70.0), GetBodygroup() );
				canReload = false;
			}
			BaseClass.Reload();
		}
		...
		if( self.m_flTimeWeaponIdle < g_Engine.time )
		{
			...
			else if( m_fRifleReload )
			{
				...
				else
				{
					...
					m_fRifleReload = false;
					self.m_flTimeWeaponIdle = g_Engine.time + (48.0/50.0);
					self.m_flNextPrimaryAttack = self.m_flNextSecondaryAttack = self.m_flNextTertiaryAttack = g_Engine.time + (48.0/49.0);
				}
			}
			else
			{
