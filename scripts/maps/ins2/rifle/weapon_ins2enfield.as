...
string V_MODEL = "models/ins2fair/wpn/enfield/v_enfield.mdl";
...
uint SLOT       	= 6;
uint POSITION   	= 7;
float RPM_AIR   	= 1.26f / 1.2f; //Rounds per minute in air
...
class weapon_ins2enfield : ScriptBasePlayerWeaponEntity, INS2BASE::WeaponBase
{
	...
	bool Deploy()
	{
		...
		{
			m_WasDrawn = true;
			return Deploy( V_MODEL, P_MODEL, DRAW_FIRST, "sniper", GetBodygroup(), (109.0/59.0) );
		}
		else
		{
			if( m_bCanBolt )
			...
			return Deploy( V_MODEL, P_MODEL, (self.m_iClip <= 0 || m_bCanBolt) ? DRAW_EMPTY : DRAW , "sniper", GetBodygroup(), (23.0/50.0) );
		}
	}
	...
	void BoltBackThink()
	{
		...
		if( self.m_iClip > 0 ) //Checking again in case the player depleted the ammo and didn't let go of the attack button
		{
			...
			self.m_flNextPrimaryAttack = self.m_flNextSecondaryAttack = self.m_flNextTertiaryAttack = WeaponTimeBase() + (38.0/46.0);
			self.m_flTimeWeaponIdle = WeaponTimeBase() + (43.0/46.0);

			SetThink( ThinkFunction( this.ShellEjectThink ) );
			self.pev.nextthink = WeaponTimeBase() + 0.37;
		}
		m_bCanBolt = false;
	}
	...
	void PrimaryAttack()
	{
		...
		if( self.m_iClip > 0 )
		{
			m_bCanBolt = true;
			SetThink( ThinkFunction( this.ShellEjectThink ) );
			self.pev.nextthink = WeaponTimeBase() + (29.0/46.0);
		}
		else
			SetThink( null );
	}
	...
	void ItemPostFrame()
	{
		...
		if( m_fRifleReload )
		{
			if( (CheckButton() || (self.m_iClip >= MAX_CLIP && m_pPlayer.pev.button & IN_RELOAD != 0)) && m_flNextReload <= g_Engine.time )
			{
				...
				self.m_flTimeWeaponIdle = g_Engine.time + (48.0/54.0);
				self.m_flNextPrimaryAttack = self.m_flNextSecondaryAttack = g_Engine.time + (48.0/54.0);
				m_fRifleReload = false;
				m_bCanBolt = false;
			}
		}
		...
		if( (m_pPlayer.m_rgAmmo( self.m_iPrimaryAmmoType ) + self.m_iClip) <= 4 || self.m_iClip >= 7 || m_fRifleReload )
		{
			if( m_reloadTimer < g_Engine.time )
			{
				...
				if( !m_fRifleReload )
				{
					...
					m_pPlayer.m_flNextAttack = (43.0/54.0); //Always uses a relative time due to prediction
					self.m_flTimeWeaponIdle = self.m_flNextSecondaryAttack = self.m_flNextPrimaryAttack = WeaponTimeBase() + (43.0/54.0);
				...
				else if( m_fRifleReload )
				{
					...
					m_pPlayer.m_flNextAttack = (37.0/56.0);
					m_flNextReload = WeaponTimeBase() + (37.0/56.0);
					self.m_flTimeWeaponIdle = self.m_flNextPrimaryAttack = self.m_flNextSecondaryAttack = WeaponTimeBase() + (37.0/56.0);
		...
		else if( (m_pPlayer.m_rgAmmo( self.m_iPrimaryAmmoType ) + self.m_iClip) >= 5 && !m_fRifleReload )
		{
			if( m_reloadTimer < g_Engine.time )
			{
				...
				if( self.m_iClip <= 1 && m_pPlayer.m_rgAmmo( self.m_iPrimaryAmmoType ) > 5 )
					Reload( MAX_CLIP, (self.m_iClip > 0) ? RELOAD_2CLIP : RELOAD_2CLIP_EMPTY, (266.0/53.0), SetBodygroup() );
				else if( self.m_iClip <= 6 && m_pPlayer.m_rgAmmo( self.m_iPrimaryAmmoType ) + self.m_iClip >= 4 )
				{
					...
					self.pev.nextthink = g_Engine.time + (172.50/53.0);

					self.SendWeaponAnim( (self.m_iClip > 0) ? RELOAD_1CLIP : RELOAD_1CLIP_EMPTY, 0, SetBodygroup() );
					m_pPlayer.m_flNextAttack = (173.0/53.0);
					self.m_flTimeWeaponIdle = self.m_flNextPrimaryAttack = self.m_flNextSecondaryAttack = WeaponTimeBase() + (173.0/53.0);
				}
		...
		if( self.m_flTimeWeaponIdle < g_Engine.time )
		{
			...
			else if( m_fRifleReload )
			{
				...
				else
				{
					...
					m_fRifleReload = false;
					m_bCanBolt = false;
					self.m_flTimeWeaponIdle = g_Engine.time + (48.0/46.0);
					self.m_flNextPrimaryAttack = self.m_flNextSecondaryAttack = g_Engine.time + (48.0/46.0);
				}
			}
			else
			{
