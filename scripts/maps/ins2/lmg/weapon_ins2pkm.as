...
string V_MODEL = "models/ins2fair/wpn/pkm/v_pkm.mdl";
...
uint SLOT       	= 7;
uint POSITION   	= 14;
float RPM_AIR   	= 1.2 * 652; //Rounds per minute in air
float RPM_WTR   	= 1.2 * 522; //Rounds per minute in water
...
class weapon_ins2pkm : ScriptBasePlayerWeaponEntity, INS2BASE::WeaponBase, INS2BASE::BipodWeaponBase
{
	...
	bool Deploy()
	{
		...
		{
			m_WasDrawn = true;
			return Deploy( V_MODEL, P_MODEL, DRAW_FIRST, "saw", GetBodygroup(), (119.0/59.0) );
		}
		else
			return Deploy( V_MODEL, P_MODEL, DRAW, "saw", GetBodygroup(), (42.0/59.0) );
	}
	...
	{
		if( WeaponADSMode == INS2BASE::IRON_IN )
			DeployBipod( BIPOD_IRON_IN, BIPOD_IRON_OUT, GetBodygroup(), (68.0/50.0), (80.0/50.0) );
		else
			DeployBipod( BIPOD_IN, BIPOD_OUT, GetBodygroup(), (68.0/50.0), (80.0/50.0) );
	}
	...
	void Reload()
	{
		...
		if( m_reloadTimer < g_Engine.time )
		{
			...
			if( WeaponBipodMode == INS2BASE::BIPOD_UNDEPLOYED )
			{
				if( self.m_iClip == 0 )
				{
					Reload( MAX_CLIP, RELOAD_EMPTY, (201.0/26.0), SetBodygroup() );
					
				}
				else
				{
					Reload( MAX_CLIP, RELOAD, (169.0/26.0), SetBodygroup() );
				}
			}
			else if( WeaponBipodMode == INS2BASE::BIPOD_DEPLOYED )
			{
				if( self.m_iClip == 0 )
					Reload( MAX_CLIP, BIPOD_RELOAD_EMPTY, (201.0/26.0), SetBodygroup() );
				else
					Reload( MAX_CLIP, BIPOD_RELOAD, (169.0/26.0), SetBodygroup() );
			}
			canReload = false;
		}
