...
string V_MODEL = "models/ins2fair/wpn/m60/v_m60.mdl";
...
class weapon_ins2m60 : ScriptBasePlayerWeaponEntity, INS2BASE::WeaponBase, INS2BASE::BipodWeaponBase
{
	...
	bool Deploy()
	{
		...
		{
			m_WasDrawn = true;
			return Deploy( V_MODEL, P_MODEL, DRAW_FIRST, "saw", GetBodygroup(), (89.0/48.0) );
		}
		else
			return Deploy( V_MODEL, P_MODEL, DRAW, "saw", GetBodygroup(), (58.0/54.0) );
	}
	...
	{
		if( WeaponADSMode == INS2BASE::IRON_IN )
			DeployBipod( BIPOD_IRON_IN, BIPOD_IRON_OUT, GetBodygroup(), (55.0/50.0), (53.0/50.0) );
		else
			DeployBipod( BIPOD_IN, BIPOD_OUT, GetBodygroup(), (55.0/50.0), (58.0/50.0) );
	}
		...
		if( m_reloadTimer < g_Engine.time )
		{
			...
			if( WeaponBipodMode == INS2BASE::BIPOD_UNDEPLOYED )
			{
				if( self.m_iClip > 0 && self.m_iClip < 9 )
					Reload( MAX_CLIP, RELOAD_HALF, (114.0/17.0), SetBodygroup() );
				else if( self.m_iClip == 0 )
					Reload( MAX_CLIP, RELOAD_EMPTY, (123.0/17.0), SetBodygroup() );
				else
					Reload( MAX_CLIP, RELOAD, (156.0/25.0), SetBodygroup() );
			}
			else if( WeaponBipodMode == INS2BASE::BIPOD_DEPLOYED )
			{
				if( self.m_iClip > 0 && self.m_iClip < 9 )
					Reload( MAX_CLIP, BIPOD_RELOAD_HALF, (113.0/17.0), SetBodygroup() );
				else if( self.m_iClip == 0 )
					Reload( MAX_CLIP, BIPOD_RELOAD_EMPTY, (123.0/17.0), SetBodygroup() );
				else
					Reload( MAX_CLIP, BIPOD_RELOAD, (154.0/25.0), SetBodygroup() );
			}
			canReload = false;
		}
