...
string V_MODEL = "models/ins2fair/wpn/rpk/v_rpk.mdl";
...
float RPM_WTR   	= 1.2 * 450; //Rounds per minute in water
...
class weapon_ins2rpk : ScriptBasePlayerWeaponEntity, INS2BASE::WeaponBase, INS2BASE::BipodWeaponBase
{
	...
	bool Deploy()
	{
		...
		if( m_WasDrawn == false )
		{
			...
			return Deploy( V_MODEL, P_MODEL, DRAW, "saw", GetBodygroup(), (30.0/51.0) );
	}
	...
	{
		if( WeaponADSMode == INS2BASE::IRON_IN )
			DeployBipod( BIPOD_IRON_IN, BIPOD_IRON_OUT, GetBodygroup(), (45.0/50.0), (58.0/50.0) );
		else
			DeployBipod( BIPOD_IN, BIPOD_OUT, GetBodygroup(), (45.0/50.0), (58.0/50.0) );
	}
	...
	{
		if( WeaponBipodMode == INS2BASE::BIPOD_UNDEPLOYED )
			ChangeFireMode( (WeaponADSMode == INS2BASE::IRON_IN) ? IRON_FIRESELECT : FIRESELECT, GetBodygroup(), INS2BASE::SELECTFIRE_AUTO, INS2BASE::SELECTFIRE_SEMI, (35.0/60.0) );
		else if( WeaponBipodMode == INS2BASE::BIPOD_DEPLOYED )
			ChangeFireMode( (WeaponADSMode == INS2BASE::IRON_IN) ? BIPOD_IRON_FIRESELECT : BIPOD_FIRESELECT, GetBodygroup(), INS2BASE::SELECTFIRE_AUTO, INS2BASE::SELECTFIRE_SEMI, (35.0/60.0) );
		...
		{
			if( WeaponBipodMode == INS2BASE::BIPOD_UNDEPLOYED )
				(self.m_iClip == 0) ? Reload( MAX_CLIP, RELOAD_EMPTY, (180.0/42.0), GetBodygroup() ) : Reload( MAX_CLIP, RELOAD, (138.0/42.0), GetBodygroup() );
			else if( WeaponBipodMode == INS2BASE::BIPOD_DEPLOYED )
				(self.m_iClip == 0) ? Reload( MAX_CLIP, BIPOD_RELOAD_EMPTY, (180.0/42.0), GetBodygroup() ) : Reload( MAX_CLIP, BIPOD_RELOAD, (145.0/42.0), GetBodygroup() );
			canReload = false;
		}
