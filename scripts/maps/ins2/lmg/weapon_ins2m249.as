...
string V_MODEL = "models/ins2fair/wpn/m249/v_m249.mdl";
...
uint SLOT       	= 7;
uint POSITION   	= 13;
float RPM_AIR   	= 1.2 * 775; //Rounds per minute in air
...
class weapon_ins2m249 : ScriptBasePlayerWeaponEntity, INS2BASE::WeaponBase, INS2BASE::BipodWeaponBase
{
	...
	bool Deploy()
	{
		...
		{
			m_WasDrawn = true;
			return Deploy( V_MODEL, P_MODEL, DRAW_FIRST, "saw", GetBodygroup(), (78.0/48.0) );
		}
		else
	...
	{
		if( WeaponADSMode == INS2BASE::IRON_IN )
			DeployBipod( BIPOD_IRON_IN, BIPOD_IRON_OUT, GetBodygroup(), (51.0/50.0), (51.0/50.0) );
		else
			DeployBipod( BIPOD_IN, BIPOD_OUT, GetBodygroup(), (51.0/50.0), (51.0/30.0) );
	}
	...
	void Reload()
	{
		...
		if( m_reloadTimer < g_Engine.time )
		{
			...
			{
				if( self.m_iClip > 16 )
					Reload( MAX_CLIP, RELOAD, (142.0/24.0), SetBodygroup() );
				else if( self.m_iClip <= 0 )
					Reload( MAX_CLIP, RELOAD_EMPTY, (174.0/26.0), SetBodygroup() );
				else
					Reload( MAX_CLIP, RELOAD_HALF, (155.0/26.0), SetBodygroup() );
			}
			else if( WeaponBipodMode == INS2BASE::BIPOD_DEPLOYED )
			{
				if( self.m_iClip > 16 )
					Reload( MAX_CLIP, BIPOD_RELOAD, (142.0/24.0), SetBodygroup() );
				else if( self.m_iClip <= 0 )
					Reload( MAX_CLIP, BIPOD_RELOAD_EMPTY, (171.0/26.0), SetBodygroup() );
				else
					Reload( MAX_CLIP, BIPOD_RELOAD_HALF, (155.0/26.0), SetBodygroup() );
			}
			canReload = false;
		}
