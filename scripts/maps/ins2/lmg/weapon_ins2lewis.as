...
string V_MODEL = "models/ins2fair/wpn/lewis/v_lewis.mdl";
...
class weapon_ins2lewis : ScriptBasePlayerWeaponEntity, INS2BASE::WeaponBase, INS2BASE::BipodWeaponBase
{
	...
	bool Deploy()
	{
		...
		{
			m_WasDrawn = true;
			return Deploy( V_MODEL, P_MODEL, DRAW_FIRST, "saw", GetBodygroup(), (109.0/56.0) );
		}
		else
			return Deploy( V_MODEL, P_MODEL, (self.m_iClip > 0) ? DRAW : DRAW_EMPTY, "saw", GetBodygroup(), (42.0/56.0) );
	}
	...
	void Reload()
	{
		...
		{
			if( WeaponBipodMode == INS2BASE::BIPOD_UNDEPLOYED )
				(self.m_iClip == 0) ? Reload( MAX_CLIP, RELOAD_EMPTY, (256.0/52.0), GetBodygroup() ) : Reload( MAX_CLIP, RELOAD, (225.0/52.0), GetBodygroup() );
			else if( WeaponBipodMode == INS2BASE::BIPOD_DEPLOYED )
				(self.m_iClip == 0) ? Reload( MAX_CLIP, BIPOD_RELOAD_EMPTY, (256.0/52.0), GetBodygroup() ) : Reload( MAX_CLIP, BIPOD_RELOAD, (225.0/52.0), GetBodygroup() );
			canReload = false;
		}
