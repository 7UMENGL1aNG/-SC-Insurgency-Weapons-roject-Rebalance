...
string V_MODEL = "models/ins2fair/wpn/knuk/v_knuk.mdl";
...
class weapon_ins2knuckles : ScriptBasePlayerWeaponEntity, INS2BASE::WeaponBase, INS2BASE::MeleeWeaponBase
{
	...
	bool Deploy()
	{
		...
		if( m_WasDrawn == false )
		{
			...
			if( Math.RandomLong( 0, 1 ) < 0.5 )
				return Deploy( V_MODEL, P_MODEL, DRAW_FIRST, "crowbar", GetBodygroup(), (118.0/51.0) );
			else
				return Deploy( V_MODEL, P_MODEL, DRAW_ALT, "crowbar", GetBodygroup(), (49.0/51.0) );
		}
		else
		{
			return Deploy( V_MODEL, P_MODEL, DRAW, "crowbar", GetBodygroup(), (28.0/51.0) );
		}
	}
	...
	void SecondaryAttack()
	{
		if( self.m_flNextSecondaryAttack < g_Engine.time )
		{
			...
			self.m_flTimeWeaponIdle = self.m_flNextSecondaryAttack = g_Engine.time + (82.0/30.0*0.1);
		}
	}

	void DoHeavyAttack()
	{
		Smack( 38.0, (18.0/35.0), DAMAGE, (38.0/35.0), flsh_Sounds[Math.RandomLong( 0, flsh_Sounds.length() - 1)], wrld_Sounds[Math.RandomLong( 0, wrld_Sounds.length() - 1)], DMG_SLASH | DMG_CLUB | DMG_ALWAYSGIB );
