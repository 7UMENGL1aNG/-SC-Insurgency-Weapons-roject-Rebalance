...
string V_MODEL = "models/ins2fair/wpn/webley/v_webley.mdl";
...
uint SLOT       	= 1;
uint POSITION   	= 12;
float RPM_AIR   	= 0.16f / 1.2f; //Rounds per minute in air
float RPM_WTR   	= 0.19f / 1.2f; //Rounds per minute in water
...
class weapon_ins2webley : ScriptBasePlayerWeaponEntity, INS2BASE::WeaponBase
{
	...
	bool Deploy()
	{
		...
		if( m_WasDrawn == false )
		{
			...
			if( Math.RandomLong( 0, 1 ) < 0.5 )
				return Deploy( V_MODEL, P_MODEL, DRAW_FIRST, "python", GetBodygroup(), (50.0/48.0) );
			else
				return Deploy( V_MODEL, P_MODEL, DRAW_RARE, "python", GetBodygroup(), (50.0/48.0) );
		}
		else
			return Deploy( V_MODEL, P_MODEL, DRAW, "python", GetBodygroup(), (15.0/56.0) );
	}
	...
	void Reload()
	{
		...
		{
			Reload( MAX_CLIP, RELOAD, (120.0/46.0), SetBodygroup() );
			canReload = false;
		}
