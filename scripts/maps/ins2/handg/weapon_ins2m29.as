...
string V_MODEL = "models/ins2fair/wpn/m29/v_m29.mdl";
...
uint SLOT       	= 1;
uint POSITION   	= 14;
float RPM_AIR   	= 1.0f / 1.2f; //Rounds per minute in air
...
class weapon_ins2m29 : ScriptBasePlayerWeaponEntity, INS2BASE::WeaponBase
{
	...
	bool Deploy()
	{
		...
		{
			m_WasDrawn = true;
			return Deploy( V_MODEL, P_MODEL, DRAW_FIRST, "python", GetBodygroup(), (35.0/52.0) );
		}
		else
		{
			if( m_bCanBolt )
			{
				...
				self.pev.nextthink = g_Engine.time + (14.0/45.0);
				return Deploy( V_MODEL, P_MODEL, DRAW_TOHAMMER, "python", GetBodygroup(), (15.0/52.0) );
			}
			else
			{
				return Deploy( V_MODEL, P_MODEL, (self.m_iClip == 0) ? DRAW_EMPTY : DRAW, "python", GetBodygroup(), (15.0/52.0) );
			}
		}
	}
	...
	void HammerBackThink()
	{
		...
		if( self.m_iClip > 0 ) //Checking again in case the player depleted the ammo and didn't let go of the attack button
		{
			...
			self.m_flNextPrimaryAttack = self.m_flNextSecondaryAttack = self.m_flNextTertiaryAttack = WeaponTimeBase() + (28.0/42.0);
			self.m_flTimeWeaponIdle = WeaponTimeBase() + (34.0/42.0);
		}
		m_bCanBolt = false;
	}
	...
	void ShootThink()
	{
		...
		if( self.m_iClip > 0 )
		{
			...
			self.pev.nextthink = WeaponTimeBase() + (25.0/42.0);
		}
		else
			SetThink( null );
	}
		...
		if( m_reloadTimer < g_Engine.time )
		{
			...
			if( !m_fRevolverReload )
			{
				if( self.m_iClip <= 0 )
				{
					...
					m_pPlayer.m_flNextAttack = (99.0/56.0); //Always uses a relative time due to prediction
					self.m_flTimeWeaponIdle = self.m_flNextPrimaryAttack = self.m_flNextSecondaryAttack = WeaponTimeBase() + (100.0/56.0);
				}
				else
				{
					self.SendWeaponAnim( RELOAD_START, 0, GetBodygroup() );

					m_pPlayer.m_flNextAttack = (75.0/56.0); //Always uses a relative time due to prediction
					self.m_flTimeWeaponIdle = self.m_flNextPrimaryAttack = self.m_flNextSecondaryAttack = WeaponTimeBase() + (76.0/56.0);
				}
				SetThink( ThinkFunction( this.RemoveAmmoThink ) );
				self.pev.nextthink = WeaponTimeBase() + (50.0/56.0);
			...
			else if( m_fRevolverReload )
			{
				...
				m_flNextReload = WeaponTimeBase() + (38.0/56.0);
				self.m_flTimeWeaponIdle = self.m_flNextPrimaryAttack = self.m_flNextSecondaryAttack = WeaponTimeBase() + (38.0/56.0);
