...
string V_MODEL = "models/ins2fair/wpn/pm/v_pm.mdl";
...
uint SLOT       	= 1;
uint POSITION   	= 4;
float RPM_AIR   	= 0.1f / 1.2f; //Rounds per minute in air
float RPM_WTR   	= 0.2f / 1.2f; //Rounds per minute in water
...
class weapon_ins2makarov : ScriptBasePlayerWeaponEntity, INS2BASE::WeaponBase
{
	...
	void Reload()
	{
		...
		{
			(self.m_iClip == 0) ? Reload( MAX_CLIP, RELOAD_EMPTY, (85.0/49.0), SetBodygroup() ) : Reload( MAX_CLIP, RELOAD, (80.0/49.0), SetBodygroup() );
			canReload = false;
		}
