...
string V_MODEL = "models/ins2fair/wpn/m1911/v_m1911.mdl";
...
uint SLOT       	= 1;
uint POSITION   	= 5;
float RPM_AIR   	= 0.165f / 1.2f; //Rounds per minute in air
float RPM_WTR   	= 0.265f / 1.2f; //Rounds per minute in water
...
class weapon_ins2m1911 : ScriptBasePlayerWeaponEntity, INS2BASE::WeaponBase
{
	...
	bool Deploy()
	{
		...
		{
			m_WasDrawn = true;
			return Deploy( V_MODEL, P_MODEL, DRAW_FIRST, "onehanded", GetBodygroup(), (75.0/48.0) );
		}
		else
			return Deploy( V_MODEL, P_MODEL, (self.m_iClip == 0) ? DRAW_EMPTY : DRAW, "onehanded", GetBodygroup(), (15.0/56.0) );
	}
		...
		if( m_reloadTimer < g_Engine.time )
		{
			if( self.m_iClip <= 0 )
			{
				...
				if( g_PlayerFuncs.SharedRandomLong( m_pPlayer.random_seed, 1, 100 ) <= 6 )
					Reload( MAX_CLIP, RELOAD_NULL, (112.0/49.0), GetBodygroup() );
				else
					Reload( MAX_CLIP, RELOAD_EMPTY, (105.0/49.0), GetBodygroup() );
			}
			else
				Reload( MAX_CLIP, RELOAD, (83.0/49.0), GetBodygroup() );
