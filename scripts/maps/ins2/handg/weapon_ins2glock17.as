...
string V_MODEL = "models/ins2fair/wpn/g17/v_g17.mdl";
...
uint SLOT       	= 1;
uint POSITION   	= 8;
float RPM_AIR   	= 0.125f / 1.2f; //Rounds per minute in air
float RPM_WTR   	= 0.20f / 1.2f; //Rounds per minute in water
...
class weapon_ins2glock17 : ScriptBasePlayerWeaponEntity, INS2BASE::WeaponBase
{
	...
	bool Deploy()
	{
		...
		{
			m_WasDrawn = true;
			return Deploy( V_MODEL, P_MODEL, DRAW_FIRST, "onehanded", GetBodygroup(), (80.0/53.0) );
		}
		else
			return Deploy( V_MODEL, P_MODEL, (self.m_iClip == 0) ? DRAW_EMPTY : DRAW, "onehanded", GetBodygroup(), (23.0/51.0) );
	}
	...
	void Reload()
	{
		...
		if( m_reloadTimer < g_Engine.time )
		{
			if( self.m_iClip == 0 )
			{
				...
				{
					Reload( MAX_CLIP, RELOAD_EMPTY_2, (114.0/45.0), GetBodygroup() );
				}
				else
					Reload( MAX_CLIP, RELOAD_EMPTY, (88.0/45.0), GetBodygroup() );
			}
			else
			{
				Reload( MAX_CLIP, RELOAD, (70.0/45.0), GetBodygroup() );
			}
			canReload = false;
		}
