...
string V_MODEL = "models/ins2fair/wpn/deagle/v_deagle.mdl";
...
uint SLOT       	= 1;
uint POSITION   	= 10;
float RPM_AIR   	= 0.155f / 1.2f; //Rounds per minute in air
float RPM_WTR   	= 0.255f / 1.2f; //Rounds per minute in water
...
class weapon_ins2deagle : ScriptBasePlayerWeaponEntity, INS2BASE::WeaponBase
{
	...
	bool Deploy()
	{
		...
		{
			m_WasDrawn = true;
			return Deploy( V_MODEL, P_MODEL, DRAW_FIRST, "onehanded", GetBodygroup(), (110.0/80.0) );
		}
		else
			return Deploy( V_MODEL, P_MODEL, (self.m_iClip == 0) ? DRAW_EMPTY : DRAW, "onehanded", GetBodygroup(), (30.0/72.0) );
	}
	...
	void Reload()
	{
		...
		{
			(self.m_iClip == 0) ? Reload( MAX_CLIP, RELOAD_EMPTY, (165.0/84.0), GetBodygroup() ) : Reload( MAX_CLIP, RELOAD, (145.0/84.0), GetBodygroup() );
			canReload = false;
		}
