...
string V_MODEL = "models/ins2fair/wpn/m16a4/v_m16.mdl";
...
uint DAMAGE     	= 1.2 * 23;
uint DAMAGE_GL  	= 1.2 * 145;
float SPEED     	= 1400;
uint SLOT       	= 5;
uint POSITION   	= 13;
float RPM_AIR   	= 1.2 * 950;
float RPM_WTR   	= 1.2 * 700;
...
class weapon_ins2m16a4 : ScriptBasePlayerWeaponEntity, INS2BASE::WeaponBase, INS2BASE::ExplosiveBase
{
	...
	{
		if( WeaponADSMode == INS2BASE::IRON_IN )
			GrenadeLauncherSwitch( GL_IRON_IN, GL_IRON_OUT, GetBodygroup(), (25.0/50.0), (28.0/50.0) );
		else
			GrenadeLauncherSwitch( GL_IN, GL_OUT, GetBodygroup(), (25.0/50.0), (28.0/50.0) );
	}
	...
	void Reload()
	{
		if( WeaponGLMode == INS2BASE::GL_AIMED )
		{
			...
			{
				ReloadSecondary( MAX_CLIP2, GL_RELOAD, (116.0/58.0), GetBodygroup() );
		...
		{
			ChangeFireMode( (WeaponADSMode == INS2BASE::IRON_IN) ? IRON_FIRESELECT : FIRESELECT, GetBodygroup(), INS2BASE::SELECTFIRE_3XBURST, INS2BASE::SELECTFIRE_SEMI, (23.0/60.0) );
			...
			if( m_reloadTimer < g_Engine.time )
			{
				(self.m_iClip == 0) ? Reload( MAX_CLIP, RELOAD_EMPTY, (136.0/49.0), GetBodygroup() ) : Reload( MAX_CLIP, RELOAD, (115.0/49.0), GetBodygroup() );
				canReload = false;
			}
