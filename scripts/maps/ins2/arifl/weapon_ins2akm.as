...
string V_MODEL = "models/ins2fair/wpn/akm/v_akm.mdl";
...
uint DAMAGE     	= 1.2 * 27;
uint DAMAGE_GL  	= 1.2 * 130;
float SPEED     	= 1325;
uint SLOT       	= 5;
uint POSITION   	= 15;
float RPM_AIR   	= 1.2 * 600; //Rounds per minute in air
float RPM_WTR   	= 1.2 * 400; //Rounds per minute in water
...
class weapon_ins2akm : ScriptBasePlayerWeaponEntity, INS2BASE::WeaponBase, INS2BASE::ExplosiveBase
{
	...
	bool Deploy()
	{
		...
		{
			m_WasDrawn = true;
			return Deploy( V_MODEL, P_MODEL, DRAW_FIRST, "m16", GetBodygroup(), (91.0/58.0) );
		}
		else
			return Deploy( V_MODEL, P_MODEL, (WeaponGLMode == INS2BASE::GL_AIMED) ? GL_DRAW : DRAW, "m16", GetBodygroup(), (22.0/50.0) );
	}
	...
	{
		if( WeaponADSMode == INS2BASE::IRON_IN )
			GrenadeLauncherSwitch( GL_IRON_IN, GL_IRON_OUT, GetBodygroup(), (35.0/56.0), (45.0/56.0) );
		else
			GrenadeLauncherSwitch( GL_IN, GL_OUT, GetBodygroup(), (35.0/56.0), (45.0/56.0) );
	}
	...
	void Reload()
	{
		if( WeaponGLMode == INS2BASE::GL_AIMED )
		{
			...
			{
				ReloadSecondary( MAX_CLIP2, GL_RELOAD, (100.0/65.0), GetBodygroup() );
		...
		else
		{
			...
			if( m_reloadTimer < g_Engine.time )
			{
				(self.m_iClip == 0) ? Reload( MAX_CLIP, RELOAD_EMPTY, (137.0/46.0), GetBodygroup() ) : Reload( MAX_CLIP, RELOAD, (125.0/50.0), GetBodygroup() );
				canReload = false;
			}
