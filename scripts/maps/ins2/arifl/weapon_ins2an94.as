...
string V_MODEL = "models/ins2fair/wpn/an94/v_an94.mdl";
...
float RPM_BURST_AIR	= 1.2 * 1600; //Burst fire rate in air
float RPM_BURST_WTR	= 1.2 * 1000; //Burst fire rate in water
...
class weapon_ins2an94 : ScriptBasePlayerWeaponEntity, INS2BASE::WeaponBase
{
	...
	bool Deploy()
	{
		...
		{
			m_WasDrawn = true;
			return Deploy( V_MODEL, P_MODEL, DRAW_FIRST, "m16", GetBodygroup(), (60.0/48.0) );
		}
		else
			return Deploy( V_MODEL, P_MODEL, DRAW, "m16", GetBodygroup(), (25.0/51.0) );
	}
	...
	void Reload()
	{
		ChangeFireMode( (WeaponADSMode == INS2BASE::IRON_IN) ? IRON_FIRESELECT : FIRESELECT, GetBodygroup(), INS2BASE::SELECTFIRE_AUTO, INS2BASE::SELECTFIRE_2XBURST, (30.0/64.0) );
		...
		{
			(self.m_iClip == 0) ? Reload( MAX_CLIP, RELOAD_EMPTY, (170.0/52.0), GetBodygroup() ) : Reload( MAX_CLIP, RELOAD, (135.0/52.0), GetBodygroup() );
			canReload = false;
		}
