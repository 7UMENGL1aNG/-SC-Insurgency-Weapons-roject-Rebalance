...
string V_MODEL = "models/ins2fair/wpn/groza/v_groza.mdl";
...
float RPM_WTR   	= 1.2 * 550; //Rounds per minute in water
...
class weapon_ins2groza : ScriptBasePlayerWeaponEntity, INS2BASE::WeaponBase, INS2BASE::ExplosiveBase
{
	...
	bool Deploy()
	{
		...
		{
			m_WasDrawn = true;
			return Deploy( V_MODEL, P_MODEL, DRAW_FIRST, "m16", GetBodygroup(), (75.0/48.0) );
		}
		else
			return Deploy( V_MODEL, P_MODEL, (WeaponGLMode == INS2BASE::GL_AIMED) ? GL_DRAW : DRAW, "m16", GetBodygroup(), (25.0/50.0) );
	}
	...
	{
		if( WeaponADSMode == INS2BASE::IRON_IN )
			GrenadeLauncherSwitch( GL_IRON_IN, GL_IRON_OUT, GetBodygroup(), (60.0/56.0), (60.0/51.0) );
		else
			GrenadeLauncherSwitch( GL_IN, GL_OUT, GetBodygroup(), (60.0/56.0), (60.0/51.0) );
	}
	...
	void Reload()
	{
		if( WeaponGLMode == INS2BASE::GL_AIMED )
		{
			...
			{
				ReloadSecondary( MAX_CLIP2, GL_RELOAD, (85.0/58.0), GetBodygroup() );
		...
		{
			ChangeFireMode( (WeaponADSMode == INS2BASE::IRON_IN) ? IRON_FIRESELECT : FIRESELECT, GetBodygroup(), INS2BASE::SELECTFIRE_AUTO, INS2BASE::SELECTFIRE_SEMI, (37.0/70.0) );
			...
			{
				if( self.m_iClip <= 0 )
					Reload( MAX_CLIP, RELOAD_EMPTY, (175.0/52.0), GetBodygroup() );
				else
					Reload( MAX_CLIP, RELOAD, (126.0/52.0), GetBodygroup() );
