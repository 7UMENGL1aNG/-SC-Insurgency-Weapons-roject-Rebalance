...
string V_MODEL = "models/ins2fair/wpn/l85a2/v_l85a2.mdl";
...
uint SLOT       	= 5;
uint POSITION   	= 14;
float RPM_AIR   	= 1.2 * 652;
float RPM_WTR   	= 1.2 * 525;
...
class weapon_ins2l85a2 : ScriptBasePlayerWeaponEntity, INS2BASE::WeaponBase, INS2BASE::ExplosiveBase
{
	...
	bool Deploy()
	{
		...
		{
			m_WasDrawn = true;
			return Deploy( V_MODEL, P_MODEL, DRAW_FIRST, "m16", GetBodygroup(), (72.0/51.0) );
		}
		else
	...
	{
		if( WeaponADSMode == INS2BASE::IRON_IN )
			GrenadeLauncherSwitch( GL_IRON_IN, GL_IRON_OUT, GetBodygroup(), (25.0/51.0), (28.0/51.0) );
		else
			GrenadeLauncherSwitch( GL_IN, GL_OUT, GetBodygroup(), (25.0/51.0), (28.0/51.0) );
	}
	...
	void Reload()
	{
		if( WeaponGLMode == INS2BASE::GL_AIMED )
		{
			...
			{
				ReloadSecondary( MAX_CLIP2, GL_RELOAD, (116.0/54.0), GetBodygroup() );
		...
		{
			ChangeFireMode( (WeaponADSMode == INS2BASE::IRON_IN) ? IRON_FIRESELECT : FIRESELECT, GetBodygroup(), INS2BASE::SELECTFIRE_AUTO, INS2BASE::SELECTFIRE_SEMI, (23.0/60.0) );
			...
			if( m_reloadTimer < g_Engine.time )
			{
				(self.m_iClip == 0) ? Reload( MAX_CLIP, RELOAD_EMPTY, (136.0/42.0), GetBodygroup() ) : Reload( MAX_CLIP, RELOAD, (115.0/42.0), GetBodygroup() );
				canReload = false;
			}
