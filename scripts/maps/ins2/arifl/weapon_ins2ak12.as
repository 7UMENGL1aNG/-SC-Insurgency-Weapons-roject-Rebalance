...
string V_MODEL = "models/ins2fair/wpn/ak12/v_ak12.mdl";
...
uint SLOT       	= 5;
uint POSITION   	= 7;
float RPM_AIR   	= 1.2 * 680; //Rounds per minute in air
float RPM_WTR   	= 1.2 * 600; //Rounds per minute in water
float RPM_BURST_AIR	= 1.2 * 800; //Burst fire rate in air
float RPM_BURST_WTR	= 1.2 * 695; //Burst fire rate in water
...
class weapon_ins2ak12 : ScriptBasePlayerWeaponEntity, INS2BASE::WeaponBase
{
	...
	bool Deploy()
	{
		...
		{
			m_WasDrawn = true;
			return Deploy( V_MODEL, P_MODEL, DRAW_FIRST, "m16", GetBodygroup(), (61.0/50.0) );
		}
		else
	...
	void Reload()
	{
		ChangeFireMode( (WeaponADSMode == INS2BASE::IRON_IN) ? IRON_FIRESELECT : FIRESELECT, GetBodygroup(), INS2BASE::SELECTFIRE_AUTO, INS2BASE::SELECTFIRE_2XBURST, (25.0/60.00) );
		...
		{
			(self.m_iClip == 0) ? Reload( MAX_CLIP, RELOAD_EMPTY, (134.0/48.0), GetBodygroup() ) : Reload( MAX_CLIP, RELOAD, (106.0/48.0), GetBodygroup() );
			canReload = false;
		}
