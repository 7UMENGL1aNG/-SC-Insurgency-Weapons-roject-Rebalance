...
string V_MODEL = "models/ins2/ins2fair/faust/v_faust.mdl";
...
class weapon_ins2pzfaust : ScriptBasePlayerWeaponEntity, INS2BASE::WeaponBase, INS2BASE::ExplosiveBase
{
	...
	bool Deploy()
	{
		...
		{
			m_WasDrawn = true;
			return Deploy( V_MODEL, P_MODEL, DRAW_FIRST, "rpg", GetBodygroup(), (104.0/50.0) );
		}
		else
			return Deploy( V_MODEL, P_MODEL, (self.m_iClip > 0) ? DRAW : DRAW_EMPTY, "rpg", GetBodygroup(), (29.0/48.0) );
	}
	...
	void PrimaryAttack()
	{
		...
		self.m_flNextPrimaryAttack = self.m_flNextSecondaryAttack = WeaponTimeBase() + (60.0/30.0);
	...
	void Reload()
	{
		...
		{
			Reload( MAX_CLIP, RELOAD, (130.0/63.0), GetBodygroup() );
			SetThink( ThinkFunction( this.EjectClipThink ) );
			self.pev.nextthink = g_Engine.time + (20.0/63.0);
			canReload = false;
		}
