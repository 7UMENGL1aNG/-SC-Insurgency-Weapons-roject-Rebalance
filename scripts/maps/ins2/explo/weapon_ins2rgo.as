...
string V_MODEL = "models/ins2/ins2fair/rgo/v_rgo.mdl";
...
class weapon_ins2rgo : ScriptBasePlayerWeaponEntity, INS2BASE::WeaponBase, INS2BASE::ExplosiveBase
{
	...
	bool Deploy()
	{
		...
		return Deploy( V_MODEL, P_MODEL, DRAW, "gren", GetBodygroup(), (19.0/48.0) );
	}
	...
	void PrimaryAttack()
	{
		...
		self.m_flNextPrimaryAttack = self.m_flNextTertiaryAttack = self.m_flNextSecondaryAttack = WeaponTimeBase() + (50.0/66.0);
		throwtype = 0;
		self.SendWeaponAnim( HIGH_PINPULL_BAKE, 0, GetBodygroup() );

		m_bInAttack = true;
		m_fAttackStart = g_Engine.time + (50.0/66.0);
		m_flStartImpactTimer = g_Engine.time + (23.0/66.0);
		m_fExplode = g_Engine.time + TIMER;
		m_flStartThrow = g_Engine.time;

		self.m_flTimeWeaponIdle = g_Engine.time + (50.0/66.0) + (22.0/60.0);
	}
	...
	void SecondaryAttack()
	{
		...
		self.m_flNextSecondaryAttack = self.m_flNextTertiaryAttack = self.m_flNextPrimaryAttack = WeaponTimeBase() + (40.0/66.0);
		throwtype = 1;
		self.SendWeaponAnim( HIGH_PINPULL, 0, GetBodygroup() );

		m_bInAttack = true;
		m_fAttackStart = g_Engine.time + (40.0/66.0);
		m_fExplode = 0;

		self.m_flTimeWeaponIdle = g_Engine.time + (40.0/66.0) + (22.0/60.0);
	}
	...
	void TertiaryAttack()
	{
		...
		self.m_flNextTertiaryAttack = self.m_flNextPrimaryAttack = self.m_flNextSecondaryAttack = WeaponTimeBase() + (40.0/66.0);
		throwtype = 2;
		...
		self.m_flTimeWeaponIdle = g_Engine.time + (40.0/66.0) + (25.0/60.0);
	}
