...
string V_MODEL = "models/ins2/ins2fair/m79/v_m79.mdl";
...
class weapon_ins2m79 : ScriptBasePlayerWeaponEntity, INS2BASE::WeaponBase, INS2BASE::ExplosiveBase
{
	...
	bool Deploy()
	{
		...
		{
			m_WasDrawn = true;
			return Deploy( V_MODEL, P_MODEL, DRAW_FIRST, "m16", GetBodygroup(), (43.0/48.0) );
		}
		else
			return Deploy( V_MODEL, P_MODEL, (m_iAmmoMode == HIGH_EXPLOSIVE) ? DRAW : DRAW_BUCK, "m16", GetBodygroup(), (31.0/50.0) );
	}
	...
	void Reload()
	{
		...
		if( m_reloadTimer < g_Engine.time )
		{
			if( m_bIsSwitching )
			{
				...
				self.m_flTimeWeaponIdle = self.m_flNextTertiaryAttack = self.m_flNextSecondaryAttack = self.m_flNextPrimaryAttack = WeaponTimeBase() + (138.0/62.0);
				m_pPlayer.m_flNextAttack = (138.0/62.0);
				SetThink( ThinkFunction( this.SwitchThink ) );
				self.pev.nextthink = g_Engine.time + (136.0/62.0);
			...
			{
				if( m_iAmmoMode == HIGH_EXPLOSIVE )
					Reload( MAX_CLIP, Math.RandomLong( RELOAD1, RELOAD3 ), (138.0/70.0), GetBodygroup() );
				else
					ReloadSecondary( MAX_CLIP2, Math.RandomLong( RELOAD1_BUCK, RELOAD3_BUCK ), (138.0/84.0), GetBodygroup() );
			}
