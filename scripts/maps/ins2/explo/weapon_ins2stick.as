...
string V_MODEL = "models/ins2fair/wpn/m24gren/v_m24.mdl";
...
class weapon_ins2stick : ScriptBasePlayerWeaponEntity, INS2BASE::WeaponBase, INS2BASE::ExplosiveBase
{
	...
	void PrimaryAttack()
	{
		...
		self.m_flNextPrimaryAttack = self.m_flNextTertiaryAttack = self.m_flNextSecondaryAttack = WeaponTimeBase() + (67.0/78.0);
		throwtype = 0;
		self.SendWeaponAnim( HIGH_PINPULL_BAKE, 0, GetBodygroup() );

		m_bInAttack = true;
		m_fAttackStart = g_Engine.time + (63.0/78.0);
		m_fExplode = g_Engine.time + TIMER;
		m_flStartThrow = g_Engine.time;

		self.m_flTimeWeaponIdle = g_Engine.time + (67.0/78.0) + (22.0/70.0);
	}
	...
	void SecondaryAttack()
	{
		...
		self.m_flNextSecondaryAttack = self.m_flNextTertiaryAttack = self.m_flNextPrimaryAttack = WeaponTimeBase() + (52.0/70.0);
		throwtype = 1;
		self.SendWeaponAnim( HIGH_PINPULL, 0, GetBodygroup() );

		m_bInAttack = true;
		m_fAttackStart = g_Engine.time + (49.0/70.0);
		m_fExplode = 0;

		self.m_flTimeWeaponIdle = g_Engine.time + (52.0/70.0) + (50.0/70.0);
	}
	...
	void TertiaryAttack()
	{
		...
		self.m_flNextTertiaryAttack = self.m_flNextPrimaryAttack = self.m_flNextSecondaryAttack = WeaponTimeBase() + (49.0/70.0);
		throwtype = 2;
		self.SendWeaponAnim( LOW_PINPULL, 0, GetBodygroup() );

		m_bInAttack = true;
		m_fAttackStart = g_Engine.time + (47.0/70.0);
		m_fExplode = 0;

		self.m_flTimeWeaponIdle = g_Engine.time + (49.0/70.0) + (40.0/70.0);
	}
