...
string V_MODEL = "models/ins2fair/wpn/ftm2/v_m2.mdl";
...
class weapon_ins2m2 : ScriptBasePlayerWeaponEntity, INS2BASE::WeaponBase
{
	...
	bool Deploy()
	{
		...
		{
			m_WasDrawn = true;
			return Deploy( V_MODEL, P_MODEL, DRAW_FIRST, "egon", GetBodygroup(), (120.0/48.0) );
		}
		else
			return Deploy( V_MODEL, P_MODEL, DRAW, "egon", GetBodygroup(), (26.0/56.0) );
	}
	...
	void PrimaryAttack()
	{
		...
		if( m_iLoopCount == 0 )
		{
			...
			self.m_flNextPrimaryAttack = g_Engine.time + (1.0/38.0);
			self.m_flTimeWeaponIdle = g_Engine.time + (18.0/38.0);

			m_flNextSoundTime = g_Engine.time + (13.0/38.0) + 0.71f / 1.2f;

			SetThink( ThinkFunction( this.FireFlames ) );
			self.pev.nextthink = g_Engine.time + (13.0/38.0);
		...
		else if( m_iLoopCount == 1 && self.m_flTimeWeaponIdle < g_Engine.time )
		{
			...
			self.m_flNextPrimaryAttack = g_Engine.time + 0.055f / 1.2f;
			self.m_flTimeWeaponIdle = g_Engine.time + (300.0/36.0);
	...
	{
		self.m_flNextSecondaryAttack = WeaponTimeBase() + 0.1f / 1.2f;
	}

	void TertiaryAttack()
	{
		self.m_flNextTertiaryAttack = WeaponTimeBase() + 0.1f / 1.2f;
	}
	...
	void FinishFire()
	{
		...
		self.m_flNextPrimaryAttack = g_Engine.time + (20.0/42.0);
		self.m_flTimeWeaponIdle = g_Engine.time + (37.0/42.0);
