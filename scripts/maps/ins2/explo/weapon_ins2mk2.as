...
string V_MODEL = "models/ins2/ins2fair/mk2gren/v_mk2.mdl";
...
class weapon_ins2mk2 : ScriptBasePlayerWeaponEntity, INS2BASE::WeaponBase, INS2BASE::ExplosiveBase
{
	...
	void PrimaryAttack()
	{
		...
		self.m_flNextPrimaryAttack = self.m_flNextTertiaryAttack = self.m_flNextSecondaryAttack = WeaponTimeBase() + (50.0/60.0);
		throwtype = 0;
		self.SendWeaponAnim( HIGH_PINPULL_NOHANDLE, 0, GetBodygroup() );

		m_bInAttack = true;
		m_fAttackStart = g_Engine.time + (50.0/60.0);
		m_fExplode = g_Engine.time + TIMER;
		m_flStartThrow = g_Engine.time;

		self.m_flTimeWeaponIdle = g_Engine.time + (50.0/60.0) + (22.0/60.0);
	}
	...
	void SecondaryAttack()
	{
		...
		self.m_flNextSecondaryAttack = self.m_flNextTertiaryAttack = self.m_flNextPrimaryAttack = WeaponTimeBase() + (40.0/60.0);
		throwtype = 1;
		self.SendWeaponAnim( HIGH_PINPULL, 0, GetBodygroup() );

		m_bInAttack = true;
		m_fAttackStart = g_Engine.time + (40.0/60.0);
		m_fExplode = 0;

		self.m_flTimeWeaponIdle = g_Engine.time + (40.0/60.0) + (22.0/60.0);
	}
	...
	void TertiaryAttack()
	{
		...
		self.m_flNextTertiaryAttack = self.m_flNextPrimaryAttack = self.m_flNextSecondaryAttack = WeaponTimeBase() + (40.0/60.0);
		throwtype = 2;
