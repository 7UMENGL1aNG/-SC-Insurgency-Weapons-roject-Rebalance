...
string V_MODEL = "models/ins2fair/wpn/svd/v_svd.mdl";
...
uint SLOT       	= 7;
uint POSITION   	= 8;
float RPM_AIR   	= 0.215f / 1.2f; //Rounds per minute in air
float RPM_WTR   	= 0.375f / 1.2f; //Rounds per minute in water
...
class weapon_ins2dragunov : ScriptBasePlayerWeaponEntity, INS2BASE::WeaponBase
{
	...
	bool Deploy()
	{
		...
		{
			m_WasDrawn = true;
			return Deploy( V_MODEL, P_MODEL, DRAW_FIRST, "sniper", GetBodygroup(), (49.0/42.0) );
		}
		else
			return Deploy( V_MODEL, P_MODEL, (self.m_iClip == 0) ? DRAW_EMPTY : DRAW, "sniper", GetBodygroup(), (21.0/40.0) );
	}
	...
	void Reload()
	{
		...
		{
			(self.m_iClip == 0) ? Reload( MAX_CLIP, RELOAD_EMPTY, (135.0/42.0), GetBodygroup() ) : Reload( MAX_CLIP, RELOAD, (98.0/42.0), GetBodygroup() );
			canReload = false;
		}
