...
string V_MODEL = "models/ins2fair/wpn/mosin/v_mosin.mdl";
...
uint SLOT       	= 7;
uint POSITION   	= 5;
float RPM       	= 1.65f / 1.2f; //Rounds per minute in air
uint AIM_FOV    	= 20; // Below 50 hides crosshair
...
class weapon_ins2mosin : ScriptBasePlayerWeaponEntity, INS2BASE::WeaponBase
{
	...
	bool Deploy()
	{
		...
		{
			m_WasDrawn = true;
			return Deploy( V_MODEL, P_MODEL, DRAW_FIRST, "sniper", GetBodygroup(), (89.0/56.0) );
		}
		else
		{
			if( m_bCanBolt )
			{
				...
				self.pev.nextthink = g_Engine.time + (21.0/48.0);
			}

			return Deploy( V_MODEL, P_MODEL, (self.m_iClip <= 0 || m_bCanBolt) ? DRAW_EMPTY : DRAW, "sniper", GetBodygroup(), (22.0/48.0) );
		}
	}
	...
	void BoltBackThink()
	{
		...
		if( self.m_iClip > 0 ) //Checking again in case the player depleted the ammo and didn't let go of the attack button
		{
			...
			self.m_flNextPrimaryAttack = self.m_flNextSecondaryAttack = self.m_flNextTertiaryAttack = WeaponTimeBase() + (44.0/40.0);
			self.m_flTimeWeaponIdle = WeaponTimeBase() + (52.0/40.0);
	...
	void PrimaryAttack()
	{
		...
		if( self.m_iClip > 0 )
		{
			m_bCanBolt = true;
			SetThink( ThinkFunction( this.ShellEjectThink ) );
			self.pev.nextthink = WeaponTimeBase() + (33.0/40.0);
		}
		else
			SetThink( null );
	...
	void ItemPostFrame()
	{
		...
		if( m_fSniperReload )
		{
			if( (CheckButton() || (self.m_iClip >= MAX_CLIP && m_pPlayer.pev.button & IN_RELOAD != 0)) && m_flNextReload <= g_Engine.time )
			{
				...
				self.m_flTimeWeaponIdle = g_Engine.time + (50.0/49.0);
				self.m_flNextPrimaryAttack = self.m_flNextSecondaryAttack = g_Engine.time + (48.0/49.0);
				m_fSniperReload = false;
			}
		}
		...
		if( m_reloadTimer < g_Engine.time )
		{
			...
			if( !m_fSniperReload )
			{
				if( self.m_iClip <= 0 )
				{
					...
					self.pev.nextthink = WeaponTimeBase() + (30.0/49.0);

					m_pPlayer.m_flNextAttack = (43.0/49.0); //Always uses a relative time due to prediction
					self.m_flTimeWeaponIdle = self.m_flNextPrimaryAttack = self.m_flNextSecondaryAttack = WeaponTimeBase() + (43.0/49.0);
				}
				else
				{
					...
					m_pPlayer.m_flNextAttack = (43.0/49.0); //Always uses a relative time due to prediction
					self.m_flTimeWeaponIdle = self.m_flNextSecondaryAttack = self.m_flNextPrimaryAttack = WeaponTimeBase() + (43.0/49.0);
				}
			...
			else if( m_fSniperReload )
			{
				...
				self.m_flTimeWeaponIdle = self.m_flNextPrimaryAttack = self.m_flNextSecondaryAttack = WeaponTimeBase() + (37.0/53.0);
