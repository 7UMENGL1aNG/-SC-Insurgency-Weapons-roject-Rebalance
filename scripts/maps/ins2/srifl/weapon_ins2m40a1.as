...
string V_MODEL = "models/ins2fair/wpn/m40a1/v_m40a1.mdl";
...
uint SLOT       	= 7;
uint POSITION   	= 4;
float RPM       	= 1.50f / 1.2f; //Rounds per minute in air
...
class weapon_ins2m40a1 : ScriptBasePlayerWeaponEntity, INS2BASE::WeaponBase
{
	...
	bool Deploy()
	{
		...
		{
			m_WasDrawn = true;
			return Deploy( V_MODEL, P_MODEL, DRAW_FIRST, "sniper", GetBodygroup(), (70.0/48.0) );
		}
		else
		{
			if( m_bCanBolt )
			{
				...
				self.pev.nextthink = g_Engine.time + (28.0/50.0);
			}

			return Deploy( V_MODEL, P_MODEL, (self.m_iClip <= 0 || m_bCanBolt ) ? DRAW_EMPTY : DRAW, "sniper", GetBodygroup(), (29.0/50.0) );
		}
	}
	...
	void BoltBackThink()
	{
		...
		if( self.m_iClip > 0 ) //Checking again in case the player depleted the ammo and didn't let go of the attack button
		{
			...
			self.m_flNextPrimaryAttack = self.m_flNextSecondaryAttack = self.m_flNextTertiaryAttack = WeaponTimeBase() + (46.0/42.0);
			self.m_flTimeWeaponIdle = WeaponTimeBase() + (52.0/42.0);
	...
	void ItemPostFrame()
	{
		...
		if( m_fSniperReload )
		{
			if( (CheckButton() || (self.m_iClip >= MAX_CLIP && m_pPlayer.pev.button & IN_RELOAD != 0)) && m_flNextReload <= g_Engine.time )
			{
				...
				self.m_flTimeWeaponIdle = g_Engine.time + (31.0/38.0);
				self.m_flNextPrimaryAttack = self.m_flNextSecondaryAttack = g_Engine.time + (28.0/38.0);
				m_fSniperReload = false;
			}
		}
	...
	void Reload()
	{
		...
		if( m_reloadTimer < g_Engine.time )
		{
			...
			if( !m_fSniperReload )
			{
				...
				self.pev.nextthink = WeaponTimeBase() + (22.0/42.0);

				m_pPlayer.m_flNextAttack = (39.0/42.0); //Always uses a relative time due to prediction
				self.m_flTimeWeaponIdle = self.m_flNextPrimaryAttack = self.m_flNextSecondaryAttack = WeaponTimeBase() + (39.0/42.0);
			...
			else if( m_fSniperReload )
			{
				...
				m_flNextReload = WeaponTimeBase() + (39.0/56.0);
				self.m_flTimeWeaponIdle = self.m_flNextPrimaryAttack = self.m_flNextSecondaryAttack = WeaponTimeBase() + (39.0/56.0);
