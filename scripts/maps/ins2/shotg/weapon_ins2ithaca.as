...
string V_MODEL = "models/ins2fair/wpn/ithaca/v_ithaca.mdl";
...
uint DAMAGE     	= 1.2 * 10;
uint DAMAGE_MELEE 	= 12 * 20;
uint SLOT       	= 3;
uint POSITION   	= 11;
float RPM_AIR   	= 0.745f / 1.2f; //Rounds per minute in air
...
class weapon_ins2ithaca : ScriptBasePlayerWeaponEntity, INS2BASE::WeaponBase, INS2BASE::MeleeWeaponBase
{
	...
	bool Deploy()
	{
		...
		{
			m_WasDrawn = true;
			return Deploy( V_MODEL, P_MODEL, DRAW_FIRST, "shotgun", GetBodygroup(), (65.0/56.0) );
		}
		else
		{
			if( m_bCanPump )
			{
				...
				self.pev.nextthink = g_Engine.time + (24.0/50.0);
			}
			return Deploy( V_MODEL, P_MODEL, DRAW, "shotgun", GetBodygroup(), (25.0/50.0) );
		}
	}
	...
	void PumpBackThink()
	{
		...
		if( self.m_iClip > 0 ) //Checking again in case the player depleted the ammo and didn't let go of the attack button
		{
			...
			self.m_flNextPrimaryAttack = self.m_flNextSecondaryAttack = self.m_flNextTertiaryAttack = WeaponTimeBase() + (23.0/42.0);
			self.m_flTimeWeaponIdle = WeaponTimeBase() + (23.0/42.0);
	...
	void ItemPostFrame()
	{
		...
		if( m_fShotgunReload )
		{
			if( (CheckButton() || (self.m_iClip >= MAX_CLIP && m_pPlayer.pev.button & IN_RELOAD != 0)) && m_flNextReload <= g_Engine.time )
			{
				...
				self.m_flTimeWeaponIdle = g_Engine.time + (21.0/45.0);
				self.m_flNextPrimaryAttack = self.m_flNextSecondaryAttack = self.m_flNextTertiaryAttack = g_Engine.time + (21.0/45.0);
				m_fShotgunReload = false;
				m_bCanPump = false;
			}
		}
		...
		if( m_reloadTimer < g_Engine.time )
		{
			...
			if( !m_fShotgunReload )
			{
				if( self.m_iClip <= 0 )
				{
					...
					self.pev.nextthink = WeaponTimeBase() + (60.0/50.0);

					m_pPlayer.m_flNextAttack = (88.0/50.0); //Always uses a relative time due to prediction
					self.m_flTimeWeaponIdle = self.m_flNextPrimaryAttack = self.m_flNextSecondaryAttack = self.m_flNextTertiaryAttack = WeaponTimeBase() + (88.0/50.0);
				}
				else
				{
					self.SendWeaponAnim( RELOAD_START, 0, GetBodygroup() );

					m_pPlayer.m_flNextAttack = (18.0/42.0); //Always uses a relative time due to prediction
					self.m_flTimeWeaponIdle = self.m_flNextSecondaryAttack = self.m_flNextPrimaryAttack = self.m_flNextTertiaryAttack = WeaponTimeBase() + (18.0/42.0);
				}
			...
			else if( m_fShotgunReload )
			{
				...
				m_flNextReload = WeaponTimeBase() + (36.0/70.0);
				self.m_flTimeWeaponIdle = self.m_flNextPrimaryAttack = self.m_flNextSecondaryAttack = self.m_flNextTertiaryAttack = WeaponTimeBase() + (36.0/70.0);
