...
string V_MODEL = "models/ins2fair/wpn/coach/v_coach.mdl";
...
uint DAMAGEPELLET 	= 1.2 * 8;
uint DAMAGESLUG 	= 1.2 * 50;
uint SLOT       	= 3;
uint POSITION   	= 13;
...
class weapon_ins2coach : ScriptBasePlayerWeaponEntity, INS2BASE::WeaponBase
{
	...
	bool Deploy()
	{
		...
		{
			m_WasDrawn = true;
			return Deploy( V_MODEL, P_MODEL, (Math.RandomLong( 0, 1 ) > 0.5) ? DRAW_FIRST_ALT : DRAW_FIRST, "shotgun", GetBodygroup(), (40.0/48.0) );
		}
		else
			return Deploy( V_MODEL, P_MODEL, DRAW, "shotgun", GetBodygroup(), (20.0/48.0) );
	}
	...
	void Reload()
	{
		...
		if( m_reloadTimer < g_Engine.time )
		{
			...
			if ( self.m_iClip == 1 || m_pPlayer.m_rgAmmo( self.m_iPrimaryAmmoType ) <= 1 )
			{
				Reload( MAX_CLIP, RELOAD, (130.0/52.0), GetBodygroup() );
				self.pev.nextthink = g_Engine.time + 1.42;
			}
			else
			{
				Reload( MAX_CLIP, RELOAD_EMPTY, (177.0/52.0), GetBodygroup() );
