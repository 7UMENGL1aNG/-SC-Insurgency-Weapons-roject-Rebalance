...
string V_MODEL = "models/ins2fair/wpn/m590/v_m590.mdl";
...
uint SLOT       	= 3;
uint POSITION   	= 12;
float RPM_AIR   	= 0.77f / 1.2f; //Rounds per minute in air
...
class weapon_ins2m590 : ScriptBasePlayerWeaponEntity, INS2BASE::WeaponBase
{
	...
	bool Deploy()
	{
		...
		{
			m_WasDrawn = true;
			return Deploy( V_MODEL, P_MODEL, DRAW_FIRST, "shotgun", GetBodygroup(), (65.0/42.0) );
		}
		else
		{
			if( m_bCanPump )
			...
			return Deploy( V_MODEL, P_MODEL, DRAW, "shotgun", GetBodygroup(), (22.0/50.0) );
		}
	}
	...
	void PumpBackThink()
	{
		...
		if( self.m_iClip > 0 ) //Checking again in case the player depleted the ammo and didn't let go of the attack button
		{
			...
			self.m_flNextPrimaryAttack = self.m_flNextSecondaryAttack = self.m_flNextTertiaryAttack = WeaponTimeBase() + (18.0/48.0);
			self.m_flTimeWeaponIdle = WeaponTimeBase() + (24.0/48.0);
	...
	void ItemPostFrame()
	{
		...
		if( m_fShotgunReload )
		{
			if( (CheckButton() || (self.m_iClip >= MAX_CLIP && m_pPlayer.pev.button & IN_RELOAD != 0)) && m_flNextReload <= g_Engine.time )
			{
				...
				self.m_flTimeWeaponIdle = g_Engine.time + (21.0/49.0);
				self.m_flNextPrimaryAttack = self.m_flNextSecondaryAttack = g_Engine.time + (21.0/49.0);
				m_fShotgunReload = false;
				m_bCanPump = false;
			}
		}
	...
	void ShellReloadEjectThink() //For the think function used in reload
	{
		...
		self.pev.nextthink = g_Engine.time + ((79.0/64.0) - (24.0/64.0));
	}
		...
		if( m_reloadTimer < g_Engine.time )
		{
			...
			if( !m_fShotgunReload )
			{
				if( self.m_iClip <= 0 )
				{
					...
					self.pev.nextthink = WeaponTimeBase() + (24.0/64.0);

					m_pPlayer.m_flNextAttack = (101.0/64.0); //Always uses a relative time due to prediction
					self.m_flTimeWeaponIdle = self.m_flNextPrimaryAttack = self.m_flNextSecondaryAttack = WeaponTimeBase() + (101.0/64.0);
				}
				else
				{
					...
					self.m_flTimeWeaponIdle = self.m_flNextSecondaryAttack = self.m_flNextPrimaryAttack = WeaponTimeBase() + (21.0/42.0);
				}
			...
			else if( m_fShotgunReload )
			{
				...
				m_flNextReload = WeaponTimeBase() + (26.0/64.0);
				self.m_flTimeWeaponIdle = self.m_flNextPrimaryAttack = self.m_flNextSecondaryAttack = WeaponTimeBase() + (26.0/64.0);
