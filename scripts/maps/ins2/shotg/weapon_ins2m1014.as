...
string V_MODEL = "models/ins2fair/wpn/m1014/v_m1014.mdl";
...
uint SLOT       	= 3;
uint POSITION      	= 14;
float RPM_AIR   	= 0.115f / 1.2f; //Rounds per minute in air
float RPM_WTR   	= 0.215f / 1.2f; //Rounds per minute in water
...
class weapon_ins2m1014 : ScriptBasePlayerWeaponEntity, INS2BASE::WeaponBase
{
	...
	bool Deploy()
	{
		...
		{
			m_WasDrawn = true;
			return Deploy( V_MODEL, P_MODEL, DRAW_FIRST, "shotgun", GetBodygroup(), (56.0/48.0) );
		}
		else
			return Deploy( V_MODEL, P_MODEL, DRAW, "shotgun", GetBodygroup(), (22.0/48.0) );
	}
		...
		if( m_reloadTimer < g_Engine.time )
		{
			...
			if( !m_fShotgunReload )
			{
				if( self.m_iClip <= 0 )
				{
					...
					self.pev.nextthink = WeaponTimeBase() + (63.0/49.0);

					m_pPlayer.m_flNextAttack = (97.5/49.0); //Always uses a relative time due to prediction
					self.m_flTimeWeaponIdle = self.m_flNextPrimaryAttack = self.m_flNextSecondaryAttack = WeaponTimeBase() + (97.5/49.0);
				}
				else
				{
					self.SendWeaponAnim( RELOAD_START, 0, GetBodygroup() );

					m_pPlayer.m_flNextAttack = (21.0/45.0); //Always uses a relative time due to prediction
					self.m_flTimeWeaponIdle = self.m_flNextPrimaryAttack = self.m_flNextSecondaryAttack = WeaponTimeBase() + (21.0/45.0);
				}
			...
			else if( m_fShotgunReload )
			{
				...
				m_flNextReload = WeaponTimeBase() + (26.0/49.0);
				self.m_flTimeWeaponIdle = self.m_flNextPrimaryAttack = self.m_flNextSecondaryAttack = WeaponTimeBase() + (26.0/49.0);
