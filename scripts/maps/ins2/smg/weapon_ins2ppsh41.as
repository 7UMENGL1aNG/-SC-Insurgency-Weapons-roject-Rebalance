...
string V_MODEL = "models/ins2fair/wpn/ppsh41/v_ppsh41.mdl";
...
uint SLOT       	= 2;
uint POSITION   	= 12;
float RPM_AIR   	= 1.2 * 900; //Rounds per minute in air
float RPM_WTR   	= 1.2 * 775; //Rounds per minute in water
...
class weapon_ins2ppsh41 : ScriptBasePlayerWeaponEntity, INS2BASE::WeaponBase
{
	...
	bool Deploy()
	{
		...
		{
			m_WasDrawn = true;
			return Deploy( V_MODEL, P_MODEL, DRAW_FIRST, "mp5", GetBodygroup(), (47.0/48.0) );
		}
		else
			return Deploy( V_MODEL, P_MODEL, (self.m_iClip > 0) ? DRAW : DRAW_EMPTY, "mp5", GetBodygroup(), (21.0/50.0) );
	}
	...
	{
		if( self.m_iClip > 0 )
			ChangeFireMode( (WeaponADSMode == INS2BASE::IRON_IN) ? IRON_FIRESELECT : FIRESELECT, GetBodygroup(), INS2BASE::SELECTFIRE_AUTO, INS2BASE::SELECTFIRE_SEMI, (19.0/60.0) );
		else
			ChangeFireMode( (WeaponADSMode == INS2BASE::IRON_IN) ? IRON_FIRESELECT_EMPTY : FIRESELECT_EMPTY, GetBodygroup(), INS2BASE::SELECTFIRE_AUTO, INS2BASE::SELECTFIRE_SEMI, (19.0/60.0) );
		...
		{
			(self.m_iClip == 0) ? Reload( MAX_CLIP, RELOAD_EMPTY, (199.0/49.0), GetBodygroup() ) : Reload( MAX_CLIP, RELOAD, (170.0/49.0), GetBodygroup() );
			canReload = false;
		}
