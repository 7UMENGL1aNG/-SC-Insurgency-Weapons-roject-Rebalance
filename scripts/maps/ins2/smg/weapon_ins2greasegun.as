...
string V_MODEL = "models/ins2fair/wpn/grease/v_grease.mdl";
...
uint SLOT       	= 2;
uint POSITION   	= 5;
float RPM_AIR   	= 1.2 * 450; //Rounds per minute in air
float RPM_WTR   	= 1.2 * 425; //Rounds per minute in water
...
class weapon_ins2greasegun : ScriptBasePlayerWeaponEntity, INS2BASE::WeaponBase
{
	...
	bool Deploy()
	{
		...
		{
			m_WasDrawn = true;
			return Deploy( V_MODEL, P_MODEL, DRAW_FIRST, "mp5", GetBodygroup(), (74.0/51.0) );
		}
		else
			return Deploy( V_MODEL, P_MODEL, (self.m_iClip > 0) ? DRAW : DRAW_EMPTY, "mp5", GetBodygroup(), (25.0/51.0) );
	}
	...
	void Reload()
	{
		...
		{
			(self.m_iClip == 0) ? Reload( MAX_CLIP, RELOAD_EMPTY, (155.0/49.0), GetBodygroup() ) : Reload( MAX_CLIP, RELOAD, (109.0/49.0), GetBodygroup() );
			canReload = false;
		}
