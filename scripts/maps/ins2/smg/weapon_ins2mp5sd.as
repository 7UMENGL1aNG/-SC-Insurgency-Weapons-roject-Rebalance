...
string V_MODEL = "models/ins2fair/wpn/mp5sd/v_mp5sd.mdl";
...
uint SLOT       	= 2;
uint POSITION   	= 9;
float RPM_AIR   	= 1.2 * 800; //Rounds per minute in air
...
class weapon_ins2mp5sd : ScriptBasePlayerWeaponEntity, INS2BASE::WeaponBase
{
	...
	bool Deploy()
	{
		...
		{
			m_WasDrawn = true;
			return Deploy( V_MODEL, P_MODEL, DRAW_FIRST, "bow", GetBodygroup(), (90.0/53.0) );
		}
		else
			return Deploy( V_MODEL, P_MODEL, DRAW, "bow", GetBodygroup(), (25.0/50.0) );
	}
	...
	void Reload()
	{
		ChangeFireMode( (WeaponADSMode == INS2BASE::IRON_IN) ? IRON_FIRESELECT : FIRESELECT, GetBodygroup(), INS2BASE::SELECTFIRE_AUTO, INS2BASE::SELECTFIRE_SEMI, (30.0/66.0) );
		...
		{
			(self.m_iClip == 0) ? Reload( MAX_CLIP, Math.RandomLong( RELOAD_EMPTY_SLAP, RELOAD_EMPTY_PUNCH ), (150.0/49.0), GetBodygroup() ) : Reload( MAX_CLIP, RELOAD, (100.0/49.0), GetBodygroup() );
			canReload = false;
		}
