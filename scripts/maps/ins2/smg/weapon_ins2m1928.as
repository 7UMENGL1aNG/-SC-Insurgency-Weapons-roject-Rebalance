...
string V_MODEL = "models/ins2fair/wpn/m1928/v_m1928.mdl";
...
uint SLOT       	= 2;
uint POSITION   	= 11;
float RPM_AIR   	= 1.2 * 700; //Rounds per minute in air
...
class weapon_ins2m1928 : ScriptBasePlayerWeaponEntity, INS2BASE::WeaponBase
{
	...
	bool Deploy()
	{
		...
		{
			m_WasDrawn = true;
			return Deploy( V_MODEL, P_MODEL, DRAW_FIRST, "m16", GetBodygroup(), (59.0/50.0) );
		}
		else
			return Deploy( V_MODEL, P_MODEL, (self.m_iClip > 0) ? DRAW : DRAW_EMPTY, "m16", GetBodygroup(), (22.0/50.0) );
	}
	...
	{
		if( self.m_iClip > 0 )
			ChangeFireMode( (WeaponADSMode == INS2BASE::IRON_IN) ? IRON_FIRESELECT : FIRESELECT, GetBodygroup(), INS2BASE::SELECTFIRE_AUTO, INS2BASE::SELECTFIRE_SEMI, (30.0/62.0) );
		else
			ChangeFireMode( (WeaponADSMode == INS2BASE::IRON_IN) ? IRON_FIRESELECT_EMPTY : FIRESELECT_EMPTY, GetBodygroup(), INS2BASE::SELECTFIRE_AUTO, INS2BASE::SELECTFIRE_SEMI, (30.0/62.0) );
		...
		if( m_reloadTimer < g_Engine.time )
		{
			if( self.m_iClip > 0 )
			{
				...
				if( g_PlayerFuncs.SharedRandomLong( m_pPlayer.random_seed, 1, 100 ) <= 6 )
					Reload( MAX_CLIP, RELOAD_NULL, (131.0/45.0), GetBodygroup() );
				else
					Reload( MAX_CLIP, RELOAD, (178.0/56.0), GetBodygroup() );
			}
			else
				Reload( MAX_CLIP, RELOAD_EMPTY, (225.0/56.0), GetBodygroup() );
